...
function submitRSVP() {
  ...
  if (presencaSelected === 'sim') {
    ...
    document.getElementById('success-msg').textContent =
      'A gente vai sentir sua falta! Mas obrigado por avisar. 💛';
  }

  // ---- Lista dinâmica homem/mulher ----
    ...
    if (generoSelecionado === 'homem') {
      ...
      listaContainer.innerHTML = `
        <a href="https://events.vipme.com.br/2470174/497634?id_promoter=7735"
          ...
          <span>
            Lista dos Homens<br>
            <span style="font-size:0.7rem;opacity:0.8;font-weight:600;">
              Jefferson
            </span>
          </span>
...
function showGuestList() {
  ...
  confirmedGuests.forEach(g => {
    ...
    const dot = g.presenca === 'Sim'
      ? '#22D3EE'
      : '#C026D3';

    div.innerHTML = `
      <span class="guest-dot" style="background:${dot}"></span>

      <strong>${g.name}</strong>

      <span style="color:rgba(240,171,252,0.5);font-size:0.8rem;">
        &nbsp;· ${g.genero} · ${g.presenca}
      </span>
    `;
